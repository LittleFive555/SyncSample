...
{
    public class StateSyncMessageHandlers : ISyncMessageHandler
    {
        public void HandleJoin(TcpGameServer server, ClientSession session, string payload)
        {
            try
            {
                ...
                StateSyncWorldManager.Instance.AddCharacter(session.Id, name);
        ...
        public void HandlePlayerInput(TcpGameServer server, ClientSession session, string payload)
        {
            try
            {
                ...
                StateSyncWorldManager.Instance.AppendPlayerInput(session.Id, input);
            }
            catch (Exception e)
            {
